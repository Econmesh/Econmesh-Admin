...
function buildQueryString(params: OpportunityListParams): string {
  ...
  if (params.quantity_min !== undefined) search.set("quantity_min", String(params.quantity_min));
  if (params.quantity_max !== undefined) search.set("quantity_max", String(params.quantity_max));
  if (params.sort) search.set("sort", params.sort);

  return search.toString();
}

export const adminOpportunitiesService = {
  list(params?: OpportunityListParams) {
    const query = buildQueryString(params ?? {});
    return api.get<OpportunityListResponse>(`/admin/opportunities?${query}`, {
      auth: true,
    });
  },

  get(id: string) {
    return api.get<Opportunity>(`/admin/opportunities/${id}`, { auth: true });
  },

  update(id: string, body: OpportunityUpdatePayload) {
    return api.patch<Opportunity>(`/admin/opportunities/${id}`, body, { auth: true });
  },

  delete(id: string) {
    return api.delete<void>(`/admin/opportunities/${id}`, { auth: true });
  },
};
